import { executeTool, describeTool, resolveSafe, TOOL_MAP } from './tools/registry.js';
...
{{EXTRA}}`;
export async function runAgent(opts, events) {
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
        ...
        for (const call of res.toolCalls) {
            ...
            if (opts.planMode || (def.perm === 'approve' && !opts.autoApprove && !isAllowed(opts.sessionId, call.name))) {
                ...
                decision = await req.decision.catch(() => 'deny');
            }
            else {
                decision = 'allow';
                stepId = `tc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
            ...
            if (decision === 'allow_always')
                rememberAllowed(opts.sessionId, call.name);
