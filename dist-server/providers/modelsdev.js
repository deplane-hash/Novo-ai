...
export function detectFormat(p) {
    if (p.npm === '@ai-sdk/anthropic')
        return 'anthropic';
    if (p.npm === '@ai-sdk/google' || p.npm === '@ai-sdk/google-generative-ai')
        return 'gemini';
    return 'openai';
}
function normalizeBaseUrl(api, format) {
    if (!api)
        return undefined;
    const url = api.replace(/\/+$/, '');
    if (format === 'anthropic' && url.endsWith('/v1'))
        return url.slice(0, -3);
    return url;
}
const ID_MAP = { 'fireworks-ai': 'fireworks' };
const META = {
...
export async function buildCatalog(staticProviders) {
    let md;
    try {
    ...
    for (const s of staticProviders) {
        if (STATIC_ONLY.includes(s.id) && !out.some((p) => p.id === s.id))
            out.push(s);
    }
    return out;
}
...
export async function initModelsDev(staticProviders) {
    try {
