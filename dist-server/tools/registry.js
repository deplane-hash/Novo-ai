...
export function resolveSafe(ctx, input) {
...
export const TOOLS = [
    {
        name: 'read',
        description: 'Read a file and return its contents with line numbers. Use offset (1-based) and limit to read sections of large files.',
        parameters: {
            type: 'object',
            properties: {
    ...
    {
        name: 'edit',
        description: 'Apply surgical edits to an existing file using exact string replacements. Each edit replaces oldString with newString. If replaceAll is true, every occurrence is replaced.',
        parameters: {
            type: 'object',
            properties: {
                path: { type: 'string', description: 'File path, relative to workspace' },
    ...
    {
        name: 'bash',
        description: 'Run a shell command — bash on Linux/macOS, cmd (Command Prompt) on Windows. Use for git, npm, node, python, and any system task. Output is captured.',
        parameters: {
            type: 'object',
            properties: {
    ...
    {
        name: 'preview',
        description: 'Open a live preview of an HTML page (or other web UI file) in the workspace. The user sees it rendered in a browser panel that auto-refreshes as you edit it. Call this whenever you build or modify a web UI.',
        parameters: {
            type: 'object',
            properties: {
    ...
    switch (name) {
        ...
        case 'edit': {
            ...
            for (const e of edits) {
                ...
                if (!oldS)
                    throw new Error('edit: empty oldString');
                const newS = String(e.newString ?? '');
                if (e.replaceAll) {
                    if (!current.includes(oldS))
                        throw new Error(`edit: "${oldS}" not found in ${p}`);
                    ...
                    if (current.indexOf(oldS, idx + 1) !== -1)
                        throw new Error(`edit: "${oldS}" is not unique in ${p} — include more context or set replaceAll`);
        ...
        case 'webfetch': {
            ...
            if (!res.ok)
                throw new Error(`webfetch ${res.status} for ${url}`);
...
function runBash(command, timeoutMs, cwd) {
