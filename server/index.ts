...
app.post('/api/config', (req, res) => {
  ...
  if (body.activeProvider && getProvider(body.activeProvider)) config.activeProvider = body.activeProvider
  if (body.activeModel) config.activeModel = body.activeModel
  if (body.theme) config.theme = body.theme
  if (typeof body.maxTokens === 'number') config.maxTokens = Math.min(8192, Math.max(256, body.maxTokens))
  if (typeof body.temperature === 'number') config.temperature = Math.min(2, Math.max(0, body.temperature))
  if (typeof body.workspace === 'string' && body.workspace.trim()) {
    try {
      const p = fs.realpathSync(body.workspace.trim())
      if (fs.statSync(p).isDirectory()) config.workspace = p
    } catch { /* keep old workspace */ }
  }
  if (typeof body.planMode === 'boolean') config.planMode = body.planMode
  if (typeof body.autoApprove === 'boolean') config.autoApprove = body.autoApprove
  if (typeof body.onboarded === 'boolean') config.onboarded = body.onboarded
...
app.post('/api/keys/validate', async (req, res) => {
  ...
  if (!p.needsKey) return res.json({ ok: true, note: 'Local provider — no key required' })
  try {
    ...
    clearTimeout(timer)
    if (res_.ok) return res.json({ ok: true })
    let detail = `${res_.status}`
    try {
      const j = (await res_.json()) as { error?: { message?: string; code?: string } }
  ...
  if (providerId === 'ollama' || providerId === 'lmstudio') {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (models.length) return res.json({ dynamic: true, models })
      }
    } catch {
...
app.get('/api/fs/read', (req, res) => {
  ...
  try {
    ...
    if (size > 1_500_000) return res.status(413).json({ error: 'File too large to preview' })
...
app.get('/api/todos', async (_req, res) => {
  ...
  try {
    ...
    const todos: { file: string; line: number; tag: string; text: string }[] = []
  ...
  try {
    const result = await runAgent(
      ...
      },
      {
        ...
        onUsage(u) {
          usage = u
          if (!u.totalTokens) u.totalTokens = u.inputTokens + u.outputTokens
        },
    ...
    if (!errorSent && msg !== 'Stream aborted') send('error', { message: msg })
  } finally {
    res.end()
  }
})
