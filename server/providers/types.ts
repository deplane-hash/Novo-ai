...
export let MODELS: Record<string, ModelMeta> = {
  /* OpenAI */
...
let activeProviders: ProviderDefinition[] = PROVIDERS

export function getActiveProviders(): ProviderDefinition[] {
  return activeProviders
}

export function setActiveProviders(providers: ProviderDefinition[]) {
  if (Array.isArray(providers) && providers.length) activeProviders = providers
}

export function setRuntimePrices(prices: Record<string, ModelMeta>) {
  MODELS = { ...MODELS, ...prices }
}

export function getDefaultBaseUrl(id: string): string {
  const p = getProvider(id)
  if (p?.defaultBaseUrl) return p.defaultBaseUrl
  const def = {
